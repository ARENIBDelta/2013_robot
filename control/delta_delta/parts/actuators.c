...
static unsigned char states[3] = {0, 0, 0};
static unsigned int goals[3] = {19400, 19400, 19400};
static int speeds[3] = {200, 200};
static unsigned int pos[3] = {17650, 17650, 17650};
...
void actuators_raise(unsigned char index) {
	float angle = angles_risen[index];
	float goal = MIN + ((float)angle / 360 * (MAX - MIN));
	set_goal(index, goal);
}

void actuators_lower(unsigned char index) {
